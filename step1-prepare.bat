...
@ECHO SET DIR=%%~dp0> step2-compile-deps.bat
@ECHO SET DIR=%%Dir:~0,-1%%>> step2-compile-deps.bat

@ECHO CD %%DIR%%\tmp\zlib>> step2-compile-deps.bat
@ECHO MKDIR build>> step2-compile-deps.bat
@ECHO CD build>> step2-compile-deps.bat
@ECHO cmake .. -G "Visual Studio 14 Win64" -DCMAKE_INSTALL_PREFIX:PATH="%%DIR%%/tmp/install_dir/zlib">> step2-compile-deps.bat
@ECHO cmake --build . --config Release --target install>> step2-compile-deps.bat

@ECHO CD %%DIR%%\tmp\libxlsxwriter>> step2-compile-deps.bat
@ECHO MKDIR build>> step2-compile-deps.bat
@ECHO CD build>> step2-compile-deps.bat
@ECHO cmake .. -G "Visual Studio 14 Win64" -DCMAKE_INSTALL_PREFIX:PATH="%%DIR%%/tmp/install_dir/libxlsxwriter" -DZLIB_ROOT:STRING="%%DIR%%/tmp/install_dir/zlib">> step2-compile-deps.bat
@ECHO cmake --build . --config Release --target install>> step2-compile-deps.bat
@ECHO CD %%DIR%%>> step2-compile-deps.bat
@ECHO PAUSE>> step2-compile-deps.bat

@ECHO @ECHO OFF> step3-compile-final.bat
@ECHO SET DIR=%%~dp0>> step3-compile-final.bat
@ECHO SET DIR=%%Dir:~0,-1%%>> step3-compile-final.bat
@ECHO IF NOT EXIST "%%DIR%%\tmp\install_dir\libxlsxwriter" ^(>> step3-compile-final.bat
@ECHO @ECHO.>> step3-compile-final.bat
@ECHO @ECHO libxlsxwriter compilied not found - please re-run step1 script>> step3-compile-final.bat
@ECHO PAUSE>> step3-compile-final.bat
@ECHO EXIT>> step3-compile-final.bat
@ECHO ^)>> step3-compile-final.bat
@ECHO SETX PATH "%%DIR%%\build-cache\php-7.1.3-devel-VC14-x64">> step3-compile-final.bat
@ECHO XCOPY /s /y "%%DIR%%\tmp\install_dir\libxlsxwriter\include\*.*" "%%DIR%%\build-cache\libxlsxwriter/include">> step3-compile-final.bat
@ECHO XCOPY /s /y "%%DIR%%\tmp\install_dir\libxlsxwriter\lib\x64\Release\*.*" "%%DIR%%\build-cache\libxlsxwriter\lib">> step3-compile-final.bat
@ECHO @ECHO CD %%DIR%%\xlswriter^> task.bat>> step3-compile-final.bat
@ECHO @ECHO call phpize 2^>^&1^>^> task.bat>> step3-compile-final.bat
@ECHO @ECHO @ECHO OFF^>^> task.bat>> step3-compile-final.bat
@ECHO @ECHO @ECHO var PHP_SECURITY_FLAGS = 'yes';^^^>^^^> configure.js^>^> task.bat>> step3-compile-final.bat
@ECHO @ECHO call configure --with-xlswriter --with-extra-libs=%%DIR%%\build-cache\libxlsxwriter\lib --with-extra-includes=%%DIR%%\build-cache\libxlsxwriter\include --enable-debug-pack 2^>^&1^>^> task.bat>> step3-compile-final.bat
@ECHO @ECHO nmake /nologo 2^>^&1^>^> task.bat>> step3-compile-final.bat
@ECHO @ECHO PAUSE^>^> task.bat>> step3-compile-final.bat
@ECHO CD %%DIR%%\xlswriter>> step3-compile-final.bat
@ECHO %%DIR%%\build-cache\php-sdk-2.1.2\phpsdk-vc14-x64.bat -t %%DIR%%\task.bat>> step3-compile-final.bat
@ECHO CD %%DIR%%>> step3-compile-final.bat
@ECHO PAUSE>> step3-compile-final.bat
